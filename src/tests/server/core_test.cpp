...
TEST_F(CoreTest, GetTradeModules) {
  ...
  ASSERT_NE(trade_modules.find(std::make_pair("USD", "RUB")),
            trade_modules.end());
  ASSERT_NE(trade_modules.find(std::make_pair("RUB", "USD")),
            trade_modules.end());
}
...
TEST_F(CoreTest, GetUserName) {
  ...
  ASSERT_EQ(core.GetUserName(user_id), username);
  ASSERT_THROW(core.GetUserName(999), std::runtime_error);
}
...
TEST_F(CoreTest, GetPeer) {
  ...
  ASSERT_THROW(core.GetPeer(999), std::runtime_error);
}
